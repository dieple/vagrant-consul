...
Vagrant Puppet Consul is a multi-VM [Vagrant](http://vagrantup.com/)-based Puppet
development environment (with consul discovery service built-in) used for creating and testing new modules outside
...
* consul1.example.com_ - the first Consul server machine
* consul2.example.com_ - the second Consul server machine
* consul3.example.com_ - the third Consul server machine
* _client1.example.com_ - the first Puppet/Consul client machine
* _client2.example.com_ - the second Puppet/Consul client machine
...
To use Vagrant Puppet Consul, you must have the following items installed and working:
...
should be good to clone this repo and go:
    ...
    $ vagrant box list
    precise64
    $ git clone https://github.com/dieple/vagrant-consul.git
    $ cd vagrant-consul/
...
1. The Puppet server daemon will be installed and enabled on the master
   machine.
2. Three consul servers will be created   
3. The Puppet/Consul client agent will be installed and enabled on all three machines.
4. A host-only network will be set up with all machines knowing how to
   communicate with each other.
5. All client certificate requests will be automatically signed by the master
   server.
6. The master server will utilize the `nodes.pp` file and `modules/` directory
...
Vagrant Puppet Consul is provided under the terms of [The MIT
